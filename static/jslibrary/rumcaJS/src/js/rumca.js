...
var RumcaJS = function(settings , elem) {
    ...
    this.appendBars = function(data,newbarsText) {
        console.log(newbarsText)
        if (newbarsText) {}
        var $bars = this.elem.find('.chart__bars');
        data.forEach(function(v,i) {
            console.log(v,newbarsText[i])
            var bar = '<div class="bar__container" data-value="' + v + '" style="width:' + v + '%">' +
                        '<span class="bar"><span class="text">'+newbarsText[i]+'</span></span></div>';
            $bars.append(bar);
        });
    ...
    this.appendAll = function(data) {
        ...
        this.appendBars(data.bars,data.axisXText);
    };
    ...
    this.addSortedData = function(data, desc) {
        if(desc){
            data.bars.reverse();
            data.axisY.reverse();
            data.barsText.reverse();
        }

        this.resetBars();
        this.appendBars(data.bars,data.barsText);
        this.resetAxisY();
        this.appendAxisY(data.axisY);
    };
